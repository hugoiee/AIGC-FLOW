...
`resolveImageRefs` 把徽章换成占位符 `IMAGE_REF_PLACEHOLDER`（当前是 `<<<image>>>`），
并把被引用的图按徽章出现顺序排到 `image_list` 前面（占位符按位置对应队列里的图，
同一张引用两次就在队列里出现两次），没被引用的连入图按原顺序补在后面。
**占位符格式是按用户口述定的，内网联调后若要改成带序号（如 `<<<image1>>>`），
只改 `image-ref.ts` 的 `placeholderOf` 一处。** 图片徽章不随连线自动插入（@ 是显式
动作），断线时移除、手动删掉不补回，和文本徽章同一套挂载守卫。视频节点的首尾帧模式
不重排 image_list（两张图的位置本身就是首帧 / 尾帧语义）。
