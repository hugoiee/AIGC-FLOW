...
- 单选下载：图像 / 视频生成节点单击后结果区右侧浮出功能面板（`node-action-panel.tsx`，
  下载 + 全屏），单个下载走 `lib/download.ts` 的 `downloadItemOf`。**媒体节点还没有这个
  面板**，单选一张上传素材仍只能双击在新标签页打开；要加的话把同一个组件挂到
  `media-node.tsx` 的图片容器上即可。多选工具条的批量下载阈值（`SELECTION_TOOLBAR_MIN = 2`）
  不要动 —— 排布、编组那几个按钮对单个节点没意义。
